(function() {
    console.log("App script loaded. Initializing fail-safe configuration routine...");
    ...
    // Check for library state loop
    function initializeHub() {
        // Look for the library under standard naming structures
        const targetLib = window.Supabase || window.supabase || (typeof Supabase !== 'undefined' ? Supabase : null);

        if (targetLib && typeof targetLib.createClient === 'function') {
            console.log("Supabase core library discovered! Setting up client...");
            mySupabaseClient = targetLib.createClient(SUPABASE_URL, SUPABASE_KEY);
            runApplicationLogic();
        } else {
            attempts++;
            if (attempts < 50) { // Wait up to 5 seconds for network download
                setTimeout(initializeHub, 100);
            } else {
                console.error("Critical Failure: Supabase package could not be fetched by browser interface.");
            }
        }
    }

    // Main App execution loop (Runs only when library is 100% verified)
    function runApplicationLogic() {
        console.log("Constructing DOM event mappings...");
        ...
        // Live Security Handshake Engine
